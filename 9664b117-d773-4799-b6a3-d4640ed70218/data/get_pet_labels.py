...
def get_pet_labels(image_dir):
    ...
    for filename in filenames:
        # Skip hidden files like .DS_Store on macOS or other system files
        if filename.startswith('.'):
            continue

        # Extract the pet label from the filename
        # Convert filename to lowercase and split by '_'
        # Join words with a space if the label has multiple parts
        label = ' '.join([word for word in filename.lower().split('_') if word.isalpha()])

        # Add to the dictionary only if the filename is not already a key
        if filename not in results_dic:
            results_dic[filename] = [label]
        else:
            print(f"Warning: Duplicate filename '{filename}' found. Skipping.")

    # Return the dictionary with image filename as key and pet label as value
    return results_dic
